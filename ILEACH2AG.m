...
for r=0:1:rmax
  ...
  %Operation for epoch
    ...
    for i=1:1:cluster-1
        t=C(i);
        j=i;
        while j>1 && C(j).distance < C(j-1).distance
            t=C(j);
            C(j)=C(j-1);
            C(j-1)=t;
            j=j-1;
    ...
    while i<cluster
        C(i).parent= n+1;
        for j=1:1:lo
            d1=sqrt( (C(j).xd-C(i).xd)^2 + (C(j).yd-C(i).yd)^2 );
            d2=C(i).distance;
            if d1<d2
                C(i).parent=C(j).id;
            end
    ...
    for i=1:1:cluster-1
        disp(int2str(C(i).parent));
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
    for z=1:1:cluster-1
        d2= sqrt( (S(C(z).id).xd-(S(n+1).xd) )^2 + (S(C(z).id).yd-S(n+1).yd )^2 );
        if d2>do
            C(z).AG = (ETX)*(4000)  + Emp*4000*( d2 * d2 * d2 *d2 );
        end
        if d2 <= do
            C(z).AG = (ETX)*(4000)  + Efs*4000*( d2 * d2 );
        end
        C(z).min_en=C(z).AG;
    end
    for z=1:1:cluster-1
        ...
        for y=1:1:cluster-1
            ...
            db=sqrt( (S(C(z).id).xd-(S(C(y).id).xd) )^2 + (S(C(z).id).yd-(S(C(y).id).yd )^2 ));
            if db>do
                inbw = (ETX +EDA)*(4000)  + Emp*4000*( db * db * db *db );
            end
            if db <= do
                inbw = (ETX +EDA)*(4000)  + Efs*4000*( db * db );
            end
            if C(z).AG> C(y).AG + inbw
                C(z).AG= C(y).AG + inbw;
                C(z).parent=y;
                C(z).im_en=inbw;
            end
    ...
    %Assigning Associated Cluster Head to Normal Nodes
    for i=1:1:n
        if ( S(i).type=='N' && S(i).E>0 )
            if(cluster-1>=1)
                ...
                if(min_dis>0)
                    ...
                    %%%%%%%%%%%%%%%%%%%%
                    if min_dis > do
                        e=(ETX+EDA)*(4000)  + Emp*4000*( min_dis  * min_dis *min_dis*min_dis );
                    end
                    if min_dis <= do
                        e=(ETX+EDA)*(4000)  + Efs*4000*( min_dis  * min_dis );
                    end
                    zmin=0;
                    
                    for z=1:1:cluster-1
                        if z==min_dis_cluster
                            break;
                        end
                        d1=sqrt( (S(C(min_dis_cluster).id).xd-(S( C(z).id ).xd) )^2 + (S(C(min_dis_cluster).id).yd-(S(C(z).id).yd) )^2 );
                        if d1>do
                            e1 = (ETX)*(4000)  + Emp*4000*( d1 * d1 * d1 *d1 );
                        end
                        if d1 <= do
                            e1 = (ETX)*(4000)  + Efs*4000*( d1 * d1 );
                        end
                        e1=e1+C(z).AG;
                        if e1 < e
                             e=e1;
                             C(min_dis_cluster).AG=e;
                             C(min_dis_cluster).im_en=e-C(z).AG;
                             zmin=z;
                             C(min_dis_cluster).parent=zmin;
                        end
                    end
                    S(C(min_dis_cluster).id).E=S(C(min_dis_cluster).id).E-C(min_dis_cluster).im_en; 
                    if zmin ~=0
                         S(C(zmin).id).E = S(C(zmin).id).E -C(zmin).im_en;
                         tmp=C(zmin).parent;
                         while tmp~=0
                            S(C(tmp).id).E= S(C(tmp).id).E -C(tmp).im_en;
                            tmp=C(tmp).parent;
                         end
                    end
...
%                     S(C(min_dis_cluster).id).E = S(C(min_dis_cluster).id).E- ( (ERX + EDA)*4000 ); 
%                     if (distance>do)
%                         S(C(min_dis_cluster).id).E=S(C(min_dis_cluster).id).E- ( (ETX+EDA)*(4000) + Emp*4000*( x*x*x*x )); 
%                     end
%                     if (distance<=do)
%                         S(C(min_dis_cluster).id).E=S(C(min_dis_cluster).id).E- ( (ETX+EDA)*(4000)  + Efs*4000*( x * x )); 
%                     end
